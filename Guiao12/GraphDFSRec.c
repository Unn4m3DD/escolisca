...
void recDfs(GraphDFSRec* g, unsigned int currentVertex);
...
void recDfs(GraphDFSRec* g, unsigned int currentVertex) {
  unsigned int* nextList = GraphGetAdjacentsTo(g->graph, currentVertex);
  for (int i = 1; i < nextList[0] + 1; i++) {
    int vertex = nextList[i];
    if (!g->marked[vertex]) {
      g->marked[vertex] = 1;
      g->predecessor[vertex] = currentVertex;
      recDfs(g, vertex);
    }
  }
}
