...
Graph* GraphCopy(const Graph* g) {
  Graph* result = GraphCreate(
      GraphGetNumVertices(g),
      GraphIsDigraph(g),
      GraphIsWeighted(g));
  for (int i = 0; i < ListGetSize(g->verticesList); i++) {
    ListMove(g->verticesList, i);
    ListMove(result->verticesList, i);
    ListInsert(result->verticesList, ListGetCurrentItem(g->verticesList));
  }
...
Graph* GraphFromFile(FILE* f) {
  ...
  */
  ...
  fscanf(f, "%u\n", &vertexCount);
  fscanf(f, "%u\n", &edgeCount);
  Graph* result = GraphCreate(vertexCount, ori, weighted);
  unsigned int v1, v2;
  float w;
  printf("%u\n", weighted);
  if (!weighted)
    for (int i = 0; i < edgeCount; i++) {
      fscanf(f, "%u %u\n", &v1, &v2);
      GraphAddEdge(result, v1, v2);
    }
  else {
    for (int i = 0; i < edgeCount; i++) {
      fscanf(f, "%u %u %f\n", &v1, &v2, &w);
      GraphAddWeightedEdge(result, v1, v2, (int)w);
    }
  }
...
unsigned short GraphCheckInvariants(const Graph* g) {
  printf("Como a funçao não é utilizada nao é explicito o objetivo");
  return 0;
}
